...
def _crawl_naver_keywords(text):
    ...
    if "music" in url:
        ...
        for data in soup.find_all("p", class_="artist"):
            if not data.get_text() in artists:
                if len(artists) >= 10:
                    break
...
def get_answer(text, user_key):
    ...
    data_header = {
        'Authorization': 'Bearer cb2d78b9764441358f93abf91cca2344',
...
def _event_handler(event_type, slack_event):
    ...
    if event_type == "app_mention":
        ...
        user_id="session"
        ret = get_answer(text,user_id)["speech"]

        keywords = _crawl_naver_keywords(ret)
        sc.api_call(
            "chat.postMessage",
            channel=channel,
            text=keywords
